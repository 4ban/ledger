...
if __name__ == '__main__':
    ...
    object = '["wrkvtxledger", "vtxdistrib", "vtxtrust", 100.13, "", "EOS6EcERTUvtafcLMtrKycWF4JX5tFHnD7d9TPfyF1pdh6tgiWPpf", "test", "nonce"]'
    ...
    # del
    object = '[]'
    out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'push', 'action', account.name, 'remove', object, '-p', 'wrkvtxledger' + '@active'])
    print('************************************************************************************************************************************************************')
    out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'get', 'table', account.name, account.name, 'entry', '-l', '10000' ])
    print(out)
