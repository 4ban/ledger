...
def setContractSteps():
    ...
    try:
        ...
        out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'set', 'code', account.name,   os.environ['HOME']+'/eclipse-workspace/ledger/wrkvtxledger/wrkvtxledger.wasm'])
        out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'set', 'abi', account.name,  os.environ['HOME']+'/eclipse-workspace/ledger/wrkvtxledger/wrkvtxledger.abi'])
    except:
...
def compileContract():
    out = subprocess.check_output(['/usr/local/eosio.cdt/bin/eosio-cpp', '-o', os.environ['HOME']+'/eclipse-workspace/ledger/wrkvtxledger/wrkvtxledger.wasm' , os.environ['HOME']+'/eclipse-workspace/ledger/wrkvtxledger/wrkvtxledger.cpp', '--abigen' ])
