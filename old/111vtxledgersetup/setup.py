...
def create111vtxledgerWallet():
    createWallet()
    out = subprocess.check_output([os.environ['CLEOS'], 'wallet', 'import', '-n', '111vtxledger', '--private-key', '5J1WYBVqoiPfoYfQxccM1Yj7fRKktVke8c1esYNWnSXEY7kJVmh'])
    out = subprocess.check_output([os.environ['CLEOS'], 'wallet', 'import', '-n', '111vtxledger', '--private-key', '5JGfwZLKGRwccDN4njvvjGjZbpbnXXKfU5enWxvUusk7cuqhMNe'])
    print(str(out))
...
def setContractSteps():
    ...
    try:
        ...
        out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'set', 'code', account.name,   os.environ['HOME']+'/eclipse-workspace/ledger/111vtxledger/111vtxledger.wasm'])
        out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'set', 'abi', account.name,  os.environ['HOME']+'/eclipse-workspace/ledger/111vtxledger/111vtxledger.abi'])
    except:
...
def setupContract():
    compileContract()
    order.contract = os.environ['HOME']+'/ledger/111vtxledger'
    setContractSteps()


def compileContract():
    out = subprocess.check_output(['/usr/local/eosio.cdt/bin/eosio-cpp', '-o', os.environ['HOME']+'/eclipse-workspace/ledger/111vtxledger/111vtxledger.wasm' , os.environ['HOME']+'/eclipse-workspace/ledger/111vtxledger/111vtxledger.cpp', '--abigen' ])
    #out = subprocess.check_output(['/usr/local/eosio.cdt/bin/eosio-cpp', '-o', os.environ['HOME'] + '/eclipse-workspace/ledger/-/111vtxledger.wast' , os.environ['HOME'] + '/eclipse-workspace/ledger/111vtxledger/111vtxledger.cpp' ])
    print(str(out))
...
if __name__ == '__main__':
    ...
    #killKeosd()
    out = subprocess.check_output(['rm', '-rf', os.environ['HOME'] + '/eosio-wallet/111vtxledger.wallet'])
    print(out)
    wallet.name = '111vtxledger'
    create111vtxledgerWallet()
    account.name = '111vtxledger'
    compileContract()
    setupContract()
    #object = f'["111vtxledger", "godaccount", "vtxdistrib", 364000000, "", "", "test","nonce"]'
    #out = subprocess.check_output([os.environ['CLEOS'],'--url', blockchain.producer, 'push', 'action', account.name, 'rcrdtfr', object, '-p', '111vtxledger' + '@active'])
