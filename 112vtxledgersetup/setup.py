...
def create112vtxledgerWallet():
    createWallet()
    out = subprocess.check_output([os.environ['CLEOS'], 'wallet', 'import', '-n', '112vtxledger', '--private-key', '5K3ssx6BdK3pgWAYzAJuUfxaHQs8zFZKdfiSAuSBdm83Sr6pKKn'])
    out = subprocess.check_output([os.environ['CLEOS'], 'wallet', 'import', '-n', '112vtxledger', '--private-key', '5KDqp7FnHmfHmrXmWx1vA9g7Kt9JXuecUNzTyrZHCDSEGjvoaJ6'])
    print(str(out))
...
def setContractSteps():
    ...
    try:
        ...
        out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'set', 'code', account.name,   os.environ['HOME']+'/eclipse-workspace/ledger/112vtxledger/112vtxledger.wasm'])
        out = subprocess.check_output([os.environ['CLEOS'], '--url', blockchain.producer, 'set', 'abi', account.name,  os.environ['HOME']+'/eclipse-workspace/ledger/112vtxledger/112vtxledger.abi'])
    except:
...
def setupContract():
    compileContract()
    order.contract = os.environ['HOME']+'/ledger/112vtxledger'
    setContractSteps()


def compileContract():
    out = subprocess.check_output(['/usr/local/eosio.cdt/bin/eosio-cpp', '-o', os.environ['HOME']+'/eclipse-workspace/ledger/112vtxledger/112vtxledger.wasm' , os.environ['HOME']+'/eclipse-workspace/ledger/112vtxledger/112vtxledger.cpp', '--abigen' ])
    #out = subprocess.check_output(['/usr/local/eosio.cdt/bin/eosio-cpp', '-o', os.environ['HOME'] + '/eclipse-workspace/ledger/-/112vtxledger.wast' , os.environ['HOME'] + '/eclipse-workspace/ledger/112vtxledger/112vtxledger.cpp' ])
    print(str(out))
...
if __name__ == '__main__':
    ...
    #killKeosd()
    out = subprocess.check_output(['rm', '-rf', os.environ['HOME'] + '/eosio-wallet/112vtxledger.wallet'])
    print(out)
    wallet.name = '112vtxledger'
    create112vtxledgerWallet()
    account.name = '112vtxledger'
    compileContract()
    setupContract()
    #object = f'["112vtxledger", "godaccount", "vtxdistrib", 364000000, "", "", "test","nonce"]'
    #out = subprocess.check_output([os.environ['CLEOS'],'--url', blockchain.producer, 'push', 'action', account.name, 'rcrdtfr', object, '-p', '112vtxledger' + '@active'])
